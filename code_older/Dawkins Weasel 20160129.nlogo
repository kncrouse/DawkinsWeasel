...
to-report go-with-selection
  let offspring-string current-string;
  let top-offspring-string current-string;
  let top-offspring-score get-score current-string;
  let uppercase-length length uppercase;
  let string-length length current-string;
  ...
  [
    set offspring-string current-string;
    let j 0;
           ; EACH CHARACTER LOOP
  ...
  set current-string top-offspring-string;
  output-type generation;
  output-type "   " output-type current-string;
...
to-report go-without-selection
  ...
  let i 0;
  while [i < length current-string-no-selection]
    [
      let mutation-probability random-float 1.0;
      if mutation-probability < mutation-rate [
        let random-letter item (random uppercase-length) uppercase;
        set current-string-no-selection replace-item i current-string-no-selection random-letter;
      ]
      set i i + 1;
    ]
  output-print word generation word " " current-string-no-selection;
  report get-score current-string-no-selection;
end
...
to initialize-string
  ...
  set current-string initial-string;
  set current-string-no-selection initial-string
end
...
Dawkins' Weasel is a NetLogo model that illustrates the principle of evolution by natural selection. It is inspired by a thought experiement presented by Richard Dawkins in his book The Blind Watchmaker. He presents the idea as follows:
...
Dawkins' Weasel is a working model of this thought experiment, demonstrating the effectiveness of selection for rapidely producing a given target phrase.
